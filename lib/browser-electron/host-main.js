...
async function primeRenderer(view) {
    try {
    ...
    try {
        switch (op) {
            ...
            case 'command': {
                ...
                const result = await withTimeout(entry.webContentsView.webContents.debugger.sendCommand(method, msg.params ?? {}), COMMAND_TIMEOUT_MS, `CDP ${method}`);
            ...
            case 'focus': {
                const viewId = msg.viewId;
                if (viewId === undefined)
                    throw new Error('focus missing viewId');
                const win = windowOfView(viewId);
                const entry = win?.views.get(viewId);
                if (win === undefined || entry === undefined)
                    throw new Error(`focus: unknown view ${viewId}`);
 ...
 */
function keepWindowsNonOccluding() {
    if (process.platform !== 'win32')
        return;
...
void app.whenReady().then(() => {
    ...
    rl.on('line', line => {
        ...
        const op = msg.op;
        queue = queue.then(() => handle(op, msg)).catch(() => { });
    });
