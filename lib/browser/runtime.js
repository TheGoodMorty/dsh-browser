...
export class BrowserRuntime extends Service {
    ...
    resolveProvider() {
        ...
        const [single] = usable;
        if (single === undefined) {
            // Two very different causes used to read identically here — nothing is
            // registered, or something is registered but cannot run — and only the
            // second one has a fix the user can apply without reinstalling.
            const registered = [...providers.values()].map(provider => provider.id);
            if (registered.length === 0) {
                throw new BrowserError('no browser provider is registered — install a browser plugin (e.g. dsh-builtin-browser) and enable it in this profile', 'BROWSER_PROVIDER_UNAVAILABLE');
            }
            throw new BrowserError(`browser provider(s) ${registered.join(', ')} are registered but report themselves unavailable — `
                + 'for dsh-builtin-browser this means no spawnable Electron binary was found; run `npx install-electron` in the active profile or set ELECTRON_PATH', 'BROWSER_PROVIDER_UNAVAILABLE');
        }
        if (usable.length > 1) {
